#!/usr/bin/env python
# coding: utf-8
"""MNIST LFNN training script matching the released 50-epoch configuration."""
...
# mnist_lfnn_l_training_log.txt (23/32 realized leaders).
...
def load_data():
    ...
    x_test = x_test[..., None]
    return x_train, y_train.astype("int32"), x_test, y_test.astype("int32")


def make_train_dataset(x, y, batch_size):
    ds = tf.data.Dataset.from_tensor_slices((x, y))
    ds = ds.shuffle(min(len(x), 20000)).batch(batch_size).prefetch(tf.data.AUTOTUNE)
    return ds
...
def build_feature_extractor(cfg: Config):
    ...
    return tf.keras.Model(inp, x, name="mnist_backbone")
...
print()
print(
    f"FINAL_BEST dataset=MNIST model=LFNN-ℓ "
