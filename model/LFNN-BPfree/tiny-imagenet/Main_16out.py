...
optimizer_13 = optim.SGD([
    {'params': net.layer13.parameters()},
    {'params': net.fc13.parameters()}
], lr=0.1, momentum=0.9, weight_decay=5e-4)  # update layer3 and 4

optimizer_14 = optim.SGD([
    {'params': net.layer14.parameters()},
    {'params': net.fc14.parameters()}
], lr=0.1, momentum=0.9, weight_decay=5e-4)  # update layer3 and 4

optimizer_15 = optim.SGD([
    {'params': net.layer15.parameters()},
    {'params': net.fc15.parameters()}
], lr=0.1, momentum=0.9, weight_decay=5e-4)  # update layer3 and 4

optimizer_16 = optim.SGD([
    {'params': net.layer16.parameters()},
...
def test(epoch):
    ...
    with torch.no_grad():
        for inputs, targets in testloader:
            inputs, targets = inputs.to(device), targets.to(device)
            outputs, _, _, _, _, _, _, _, _, _, _, _, _, _, _, _ = net(inputs)
